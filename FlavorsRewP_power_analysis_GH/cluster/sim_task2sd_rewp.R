...
for (task2_change in seq(task2_change_min, task2_change_max, by = task2_change_step)) {
  ...
  data_tib <-
    ...
    mutate(erp_raw = erp,
           erp = erp + 
             ifelse(task == "task2" & event == "gain", 
                    rnorm(n(),0,task2_change), 
                    0)) %>%
    nest(data = c(subjid,site,task,event,erp,erp_raw)) %>%
    mutate(change = paste0("task2:rewp add stddev ", task2_change)) %>%
  ...
  saveRDS(data_tib,file.path(save_path,
                             paste0("data_tib_",which_dataset,
                                    "_task2sd_rewp_",task2_change,
                                    ".rds")))
  
  
  # if (task2_change < .001) {
  #   system(
  #     paste("sbatch /fslhome/fslcollab40/ff/task2changesd_ern.sh",
  #           which_dataset,
  #           task2_change+.0002),
  #     intern = TRUE)
