...
task2_change_min <- as.numeric(parse_pars[2])
task2_change_max <- as.numeric(parse_pars[3])
task2_change_step <- as.numeric(parse_pars[4])

cat("which_dataset:", which_dataset, "\n",
    "task2_change_min:", task2_change_min, "\n",
    "task2_change_max:", task2_change_max, "\n",
    "task2_change_step:", task2_change_step, "\n")
...
save_path <- "/home/fslcollab40/nobackup/autodelete/FlavorsRewP/data_tib_task2"

# load_path <- "/Users/peterclayson/Library/CloudStorage/Box-Box/gdrive_data/FlavorsRewP/pwranalysis/data_tib"
# save_path <- "/Users/peterclayson/Library/CloudStorage/Box-Box/gdrive_data/FlavorsRewP/pwranalysis/data_tib_task2"
...
for (task2_change in seq(task2_change_min, task2_change_max, by = task2_change_step)) {
  ...
  data_tib <-
    ...
    unnest(raw_sim) %>%
    mutate(erp_raw = erp,
           erp = ifelse(task == "task2" & event == "gain", erp + task2_change, erp)) %>%
    nest(data = c(subjid,site,task,event,erp,erp_raw)) %>%
    mutate(change = paste0("task2:gain plus ", task2_change)) %>%
  ...
  saveRDS(data_tib,file.path(save_path,
                             paste0("data_tib_",which_dataset,
                                    "_task2_rewp_",task2_change,
                                    ".rds")))
}

# if (task2_change < 1) {
#   system(
#     paste("sbatch /fslhome/fslcollab40/ff/task2change_rewp.sh",
#           which_dataset,
#           task2_change+.05),
#     intern = TRUE)
# 
