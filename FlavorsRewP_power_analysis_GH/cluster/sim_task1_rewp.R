...
# save_path <- "/Users/peterclayson/Library/CloudStorage/Box-Box/gdrive_data/FlavorsRewP/pwranalysis/data_tib_task1"
...
for (task1_change in seq(task1_change_min, task1_change_max, by = task1_change_step)) {
  ...
  data_tib <-
    ...
    unnest(raw_sim) %>%
    mutate(erp_raw = erp,
           erp = ifelse(task == "task1" & event == "gain", erp + task1_change, erp)) %>%
    nest(data = c(subjid,site,task,event,erp,erp_raw)) %>%
    mutate(change = paste0("task1:gain plus ", task1_change)) %>%
  ...
  saveRDS(data_tib,file.path(save_path,
                             paste0("data_tib_",which_dataset,
                                    "_task1_rewp_",task1_change,
                                    ".rds")))
}

# if (task1_change < 1) {
#   system(
#     paste("sbatch /fslhome/fslcollab40/ff/task1change_rewp.sh",
#           which_dataset,
#           task1_change+.05),
#     intern = TRUE)
# 
